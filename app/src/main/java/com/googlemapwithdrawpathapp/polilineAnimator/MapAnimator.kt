...
class MapAnimator private constructor() {
    ...
    fun animateRoute(googleMap: GoogleMap, routeList: List<LatLng>) {
        ...
        val optionsBackground = PolylineOptions().add(routeList[0]).color(GREY).width(7f)
        backgroundPolyline = googleMap.addPolyline(optionsBackground)

        optionsForeground = PolylineOptions().add(routeList[0]).color(Color.BLACK).width(7f)
        foregroundPolyline = googleMap.addPolyline(optionsForeground)

        val percentageCompletion = ValueAnimator.ofInt(0, 100)
        percentageCompletion.duration = 4000
